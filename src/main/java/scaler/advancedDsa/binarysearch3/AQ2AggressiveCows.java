package scaler.advancedDsa.binarysearch3;

import java.util.Arrays;

public class AQ2AggressiveCows {
    //    Problem Description
...
//    The minimum distance will be 1.
    public int solve(int[] A, int B) {
        ...
        while (l <= r) {
            mid = (l + r) / 2;
            System.out.printf("l:%s | r:%s | mid:%s | ans:%s%n", l, r, mid, ans);
            if (check(A, mid, B)) {
                ans = mid;
                l = mid + 1;
            } else {
    ...
    //to check if all the cows can be placed in the same distance which covers the entire array.
    private boolean check(int[] a, long mid, int b) {
        int lastPlaced = a[0];
        int cow = 1;
        for (int i = 1; i < a.length; i++) {
            if (a[i] - lastPlaced >= mid) {
                ...
                lastPlaced = a[i];
                if (cow == b) {
                    System.out.println("returning:" + true + " | cow:" + cow);
                    return true;
                }
            }
        }
...
//        int input1B = 3;
//        System.out.println(new AQ2AggressiveCows().solve(input1A, input1B));
//
//        int[] input2A = {1, 2};
//        int input2B = 2;
//        System.out.println(new AQ2AggressiveCows().solve(input2A, input2B));
//
//        int[] input3A = {5, 17, 100, 11};//95
//        int input3B = 2;
//        System.out.println(new AQ2AggressiveCows().solve(input3A, input3B));
        ...
        int input4B = 385;
        System.out.println(new AQ2AggressiveCows().solve(input4A, input4B));
    }
}
