...
import scaler.module1.hashing1.AQ5ShaggyAndDistances;

import java.util.HashMap;
import java.util.Map;

public class AQ2ShaggyAndDistances {
//    Problem Description
...
//    Only possibility is choosing A[1] and A[2].
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new AQ2ShaggyAndDistances().solve(input1A));//3
        int[] input2A = {1, 1};
        System.out.println(new AQ2ShaggyAndDistances().solve(input2A));//1
        ...
        System.out.println(new AQ2ShaggyAndDistances().solve(input3A));//-1
    }
    ...
    public int solve(int[] A) {
        ...
        for (int i = 0; i < A.length; i++) {
            if (hashMap.containsKey(A[i])) {
                int distance = i - hashMap.get(A[i]);
